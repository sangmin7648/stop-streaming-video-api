...
class WatchVerifierItemTest {
    ...
    void add_filter_to_verifier() {
        var filter = new WatchFilter(Provider.YOUTUBE, new Property.Category("test"));
    ...
    void delete_filter_from_verifier() {
        // given
        var filter1 = new WatchFilter(Provider.YOUTUBE, new Property.Category("test1"));
        var filter2 = new WatchFilter(Provider.YOUTUBE, new Property.Category("test2"));
    ...
    void cannot_watch_video_if_video_property_not_in_whitelist_verifier() {
        ...
        var provider = Provider.YOUTUBE;
        var property = new Property.Category("test");
        var video = new Video("test-id", provider, List.of(new Property.Category("test1")));
        var sut = randomUserWhitelistVerifierWithFilter(new WatchFilter(provider, property));
    ...
    private void addMaxFilter(WatchVerifierItem sut, int maxCount) {
        for (int i = 0; i < maxCount; i++) {
            sut.addFilter(new WatchFilter(Provider.YOUTUBE, new Property.Category(String.valueOf(i))));
        }
    }
